...
function show_exam(exam) {
    ...
    if (exam.rongpai) $('.shoupai .zimo').prepend('<span>荣和</span>');
    else              $('.shoupai .zimo').append('<span>自摸</span>');

    let defen;
    if (hule && hule.defen) {
        defen = (exam.rongpai ? '荣和: ' : '自摸: ')
...
function next() {
    ...
    $('.stat').text(
        `回答数: ${stat.total}、`
        + `正确率: ${(stat.right / stat.total * 100)|0}%`);
